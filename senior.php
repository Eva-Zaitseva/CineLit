...
$tables = [
    'movies' => 'senior_movies',
    'performances' => 'senior_performances',
    'poems' => 'senior_poems',
    'audio' => 'senior_audiobooks'
];
...
<!-- Кнопки для выбора класса -->
    <form action="senior.php" method="POST" class="class_buttons">
    ...
    <div class="fil_poisk">
        <form action="senior.php" method="post" class="poisk">
        ...
        // if (!empty($searchQuery) || !empty($classFilter)): 
            ?>
            <!-- <form action="senior.php" method="post" class="reset_form">
...
<div class="cinema_container">
    ...
    <?php if (isset($results['movies']) && $results['movies']->num_rows > 0): ?>
        ...
        while ($movie = $results['movies']->fetch_assoc()) {
            ...
            echo '<a href="senior_view.php?id=' . $movie['id'] . '">';
            echo '<img src="' . $imagePath . '" alt="' . htmlspecialchars($movie['title']) . '">';
            echo '</a>';
            echo '<a href="senior_view.php?id=' . $movie['id'] . '">' . htmlspecialchars($movie['title']) . '</a>';
            echo '</div>';
        }
        ?>
        </div>
...
<?php if (isset($results['performances']) && $results['performances']->num_rows > 0): ?>
    <div class="spect_container" id="performance">
        ...
        while ($performance = $results['performances']->fetch_assoc()) {
            ...
            echo '<a href="senior_view_performance.php?id=' . $performance['id'] . '">';
            echo '<img src="' . $imagePath . '" alt="' . htmlspecialchars($performance['title']) . '">';
            echo '</a>';
            echo '<a href="senior_view_performance.php?id=' . $performance['id'] . '">' . htmlspecialchars($performance['title']) . '</a>';
            echo '</div>';
        }
        ?>
        </div>
...
<?php if (isset($results['poems']) && $results['poems']->num_rows > 0): ?>
    <div class="poem_container" id="poem">
        ...
        while ($poem = $results['poems']->fetch_assoc()) {
            ...
            echo '<a href="senior_view_poem.php?id=' . $poem['id'] . '">';
            echo '<img src="' . $imagePath . '" alt="' . htmlspecialchars($poem['title']) . '">';
            echo '</a>';
            echo '<a href="senior_view_poem.php?id=' . $poem['id'] . '">' . htmlspecialchars($poem['title']) . '</a>';
            echo '</div>';
        }
        ?>
        </div>
...
<?php if (isset($results['audio']) && $results['audio']->num_rows > 0): ?>
    <div class="audio_container" id="audio">
        ...
        while ($audiobook = $results['audio']->fetch_assoc()) {
            ...
            echo '<a href="senior_view_audiobooks.php?id=' . $audiobook['id'] . '">';
            echo '<img src="' . $imagePath . '" alt="' . htmlspecialchars($audiobook['title']) . '">';
            echo '</a>';
            echo '<a href="senior_view_audiobooks.php?id=' . $audiobook['id'] . '">' . htmlspecialchars($audiobook['title']) . '</a>';
            echo '</div>';
        }
        ?>
        </div>
